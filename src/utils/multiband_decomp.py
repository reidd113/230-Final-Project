...
def qmfrt_example():
        ...
        plt.plot(qmfwin)
        plt.title('PQMF Prototype Function (Window)')
...
def compute_pqmf(audio, block_size=64):
  """
  Implements the real-time PQMF analysis on input audio

  Args:
    audio: input audio as array of samples (expected size=(64000,))
    block_size: size of blocks for analysis (default 64)
  Returns:
    Y_tot: multiband spectral decomposition of input audio into block_size bands (expected size=(1000, 64))

  """
  n_blocks = int(len(audio)/block_size)

  Y_tot = np.zeros((n_blocks, block_size))

  for m in range(n_blocks):
    # print("Block number: ", m)
    #Analysis:
    data=audio[m*block_size:(m+1)*block_size]
    y=analysisqmf_realtime(data,Fa,block_size)
    Y_tot[m,:] = y
